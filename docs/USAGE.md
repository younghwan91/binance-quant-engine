...
키도 네트워크도 없이 돌려보려면 `binance_quant_engine.data.klines.synth_ohlcv` 로 합성 데이터를 만들면 된다. `qbe-backtest --demo` 가 쓰는 게 이것이다.

## 2. 내 전략 붙이기

[`TradingStrategy`](../src/binance_quant_engine/strategy/protocol.py) 는 PEP 544 프로토콜이라 상속이 필요 없다. 아래 메서드만 있으면 백테스터와 라이브 양쪽에서 그대로 돈다. 시그널은 `-1`(숏) / `0`(관망) / `+1`(롱).
...
가장 짧은 레퍼런스 구현은 [`demo_squeeze.py`](../src/binance_quant_engine/strategy/demo_squeeze.py) 다.
...
`binance_quant_engine.execution` 은 바이낸스 USDT-M 선물에 실제 주문을 태우는 레이어라 `python-binance` 가 필요하다.
...
- [`brackets.py`](../src/binance_quant_engine/execution/brackets.py) — 진입 직후 SL/TP/트레일링을 거래소 **Algo Order** 로 걸어둔다. 가격이 유리하게 가면 트레일링 스탑을 서버사이드에서 래칫(한 방향으로만 조임)한다. 봇 프로세스가 죽어도 청산은 거래소가 책임진다.
- [`algo_api.py`](../src/binance_quant_engine/execution/algo_api.py) — 바이낸스 Algo Order REST 래퍼.
- [`host.py`](../src/binance_quant_engine/execution/host.py) — 실행 믹스인이 기대하는 호스트 인터페이스와 `TrailConfig`.
